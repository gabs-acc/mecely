...
def _known_values(cls: type, values: dict[str, Any], section: str) -> dict[str, Any]:
    ...
    if unknown:
        ...
        raise ConfigError(f"unknown option in [{section}]: {names}")
    return values
...
def load_config(path: Path | None = None) -> tuple[MecelyConfig, Path]:
    ...
    if unknown_sections:
        raise ConfigError(f"unknown section: {', '.join(sorted(unknown_sections))}")

    ui_data = _section(data, "ui")
    palette_data = ui_data.pop("palette", {})
    if not isinstance(palette_data, dict):
        raise ConfigError("[ui.palette] must be a TOML table")
    try:
        ...
        raise ConfigError(f"invalid type in configuration: {error}") from error
