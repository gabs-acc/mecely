...
(see cases.py). Whenever it's set, the interviewer switches to strict
fidelity instead, citing only what that text says and saying so plainly
when asked about something it doesn't cover, rather than inventing a number.
"""
...
anotação, que pode ser uma pergunta, uma explicação, ou parte de uma \
recomendação. Responda apenas à anotação MAIS RECENTE (a última da lista \
abaixo).
